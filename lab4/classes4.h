...
public:
    Fridge(string name = "Unknown", string brand = "Unknown", double price = 0.0, double capacity = 0.0)
...
public:
    WashingMachine(string name = "Unknown", string brand = "Unknown", double price = 0.0, double loadCapacity = 0.0)
...
public:
    TV(string name = "Unknown", string brand = "Unknown", double price = 0.0, double screenSize = 0.0)
        : Appliance(name, brand, price), screenSize(screenSize) {}

    double getScreenSize() const {
        return screenSize;  // Исправлено на double, так как screenSize - double
    }
    void setScreenSize(double screenSize) { // Изменено на double для соответствия типу
